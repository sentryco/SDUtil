...
extension SDUtil {
   ...
   public static func hasPersistentFile(db: DBKind) -> Bool {
      ...
      let container: ModelContainer? = {
         do {
            return try db.getContainer()
         } catch {
            Swift.print("error.localizedDescription:  \(error.localizedDescription)")
            return nil
         }
      }()
      guard let container = container else { Swift.print("⚠️️ Err - no container"); return false }
      guard let url: URL = container.configurations.first?.url else { Swift.print("⚠️️ Err - no url"); return false }
